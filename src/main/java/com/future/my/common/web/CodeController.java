...
public class CodeController {
	...
	//삽입 
	@PostMapping(value="/addCode", produces="application/json; charset=UTF-8")       // post 방식으로 json 데이터 형식으로 요청 되었을때 
	public ResponseEntity<String> addCode(@RequestBody CodeVO vo){
		System.out.println(vo);
		try {
			codeService.addCode(vo);
			//ResponseEntity message와 상태코드를 리턴			
	...
	//삭제 
	@DeleteMapping(value="/deleteCode/{comm_cd}", produces="application/json; charset=UTF-8") //PathVariable 리소스의 경로의 일부를 전달받을때 
